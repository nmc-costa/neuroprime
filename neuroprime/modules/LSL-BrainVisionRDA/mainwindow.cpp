...
void MainWindow::load_config(const std::string& filename) {
	...
	try {
		...
		// maybe someday, RecView will also be implelemted...
...
void MainWindow::link_rda() {
	if (reader_thread_) {
		...
		try {
			...
			reader_thread_.reset(new std::thread(&MainWindow::read_thread, this, serverIP));
		}
		catch (std::exception & e) {
	...
	try {
		...
		while (!stop_) {
			...
			case 1:
				...
				//it_resolver
				ep = boost::asio::connect(socket_, resolver.resolve(query), ec);
				if (!ec) {
					//ep = *it_resolver;
