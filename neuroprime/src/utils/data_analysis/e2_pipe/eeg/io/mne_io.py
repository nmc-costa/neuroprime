...
# for updates go to https://github.com/palday/philistine
...
def _write_vmrk_file(vmrk_fname, eeg_fname, events):
    ...
    with codecs.open(vmrk_fname, 'w', encoding='utf-8') as fout:
        ...
        print(r'DataFile={}'.format(os.path.split(eeg_fname)[-1]), file=fout)  # noqa: E501 #eeg_fname.split(os.sep)[-1]
        print(r'', file=fout)
...
def _write_vhdr_file(vhdr_fname, vmrk_fname, eeg_fname, raw,
    ...
    with codecs.open(vhdr_fname, 'w', encoding='utf-8') as fout:
        ...
        print(r'Codepage=UTF-8', file=fout)
        print(r'DataFile={}'.format(os.path.split(eeg_fname)[-1]), file=fout)  # noqa: E501 eeg_fname.split(os.sep)[-1]
        print(r'MarkerFile={}'.format(os.path.split(vmrk_fname)[-1]), file=fout)  # noqa: E501 vmrk_fname.split(os.sep)[-1]
